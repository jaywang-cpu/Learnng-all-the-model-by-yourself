...
```
**记住：这里的是按照名字来选的，所以是左闭右闭，不管是不是切片。**
# 按行标签和列标签选择
...
```
记住：这里是按照数字来的，所以切片是左闭右开像 0；2，但是元祖就是精准选择【0，2】
# 按数字位置选择（从0开始）
df.iloc[0, 1]           # 第0行第1列
...
# 用列表选择不连续位置
df.iloc[[0, 2], [0, 2]] # 第0,2行的第0,2列

# 这个后面可以加.mean(),.sum()···
test_data.iloc[:, -1].sum()
test_data.iloc[:, -1].mean():.1%
```

## df.at[]--单个值访问（标签）
```
# 快速访问单个标量值，基于标签
value = df.at[0, 'name']    # 'Alice'
df.at[0, 'age'] = 26        # 修改值

# 比df.loc[0, 'name']更快，但只能访问单个值
# df.at[0:1, 'name']  # ❌ 不支持切片
```
## df.iat[]--单个值访问（位置）
```
# 快速访问单个标量值，基于位置
value = df.iat[0, 1]        # 第0行第1列的值
df.iat[0, 1] = 26          # 修改值

# 比df.iloc[0, 1]更快，但只能访问单个值
# df.iat[0:1, 1]  # ❌ 不支持切片
```
